...
def load_saved_attributes():
    ...
    with open("Bangalore-Housing-Price-Prediction-master\\columns.json", "r") as f:
        resp = json.load(f)
        availability_values = resp["availability_columns"]
        area_values = resp["area_columns"]
        location_values = resp["location_columns"]

    model = pickle.load(open("Bangalore-Housing-Price-Prediction-master\\banglore_home_prices_model.pickle", "rb"))
